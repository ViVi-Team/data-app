...
def load_gold_standard(xlsx_path: str) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        if not pid or not slide or pid == 'nan' or slide == 'nan':
            continue
        boxes = parse_bbox(row.get('Corrected and rotated BBOX [x1, y1, x2, y2]'))
...
def build_patient_inventory(data_folder: str):
    ...
    return dict(inventory), path_map
...
def perturb_box(box: list, sigma: float, rng: random.Random) -> list:
    """
    Expand box outward by a small positive amount.
...
def generate_perturbed_labels(
    ...
    Returns list of dicts:
      {patient_id, slide, origin_slice, boxes, notes, distance}
    """
    ...
    # Build gold lookup: patient_id -> {slice_num -> (filename, boxes, notes)}
    ...
    for pid, files in sorted(inventory.items()):
        ...
        for fname in files:
            ...
            perturbed_boxes = [perturb_box(b, sigma, rng) for b in best_boxes]
...
def main():
    ...
    inventory, path_map = build_patient_inventory(args.data_folder)
